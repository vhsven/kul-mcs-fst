...
class STLCFixTest extends FunSuite with ShouldMatchers {
	val calc = new STLCFixCalculus();
	...
	evaluateAndTypeTest(""" fix (\f : Nat -> Nat -> Nat. \x: Nat. if iszero x then (\y:Nat.y) else (\y:Nat. succ (f (pred x) y)))""", TArr(TNat,TArr(TNat,TNat)), null); 
	evaluateAndTypeTest(""" (fix (\f : Nat -> Nat -> Nat. \x: Nat. if iszero x then (\y:Nat.y) else (\y:Nat. succ (f (pred x) y)))) 3 2""", TNat, 
			Succ(Succ(Succ(Succ(Succ(Zero)))))); // 3 + 2 = 5 
}